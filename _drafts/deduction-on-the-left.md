...
Or you can just default-construct a `parser`:
...
To prevent the above, you could restrict `parser` to only be constructible by `parse`, and delete all of its copy and move operations.
...
Particularly deviant users can still take a reference to it with `auto&& p = parse()`, but they deserve whatever befals them as a result.
